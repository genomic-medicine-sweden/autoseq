...
The pipeline includes a module to call Copy Number Variants (CNVs) using `Jumble`. The CNV calling process involves several steps, including segmentation and annotation of CNVs. The annotation step utilizes a custom script `annotate_cnvs.py` to add gene-specific comments based on the cancer type and sample type (somatic or germline).

`annotate_cnvs.py` takes the CNV segmentation file generated by `Jumble` and annotates it using a curation annotation file. The annotation file contains gene-specific information that is filtered based on the provided cancer type and sample type.

```sh

annotate_cnvs.py --help

usage: annotate_cnvs.py [-h] -i INPUT -t {germline,somatic} [-c CURATION_ANNOTATION_FILE] [--cancer-type CANCER_TYPE] -o OUTPUT [-v]
...
optional arguments:
  ...
  -i INPUT, --input INPUT
                        Input CNV segmentations file
  -t {germline,somatic}, --sample-type {germline,somatic}
                        Sample type: somatic or germline
  -c CURATION_ANNOTATION_FILE, --curation-annotation-file CURATION_ANNOTATION_FILE
                        Annotation file for curation
  --cancer-type CANCER_TYPE
                        shorthand code for cancer type
  -o OUTPUT, --output OUTPUT
                        output segmentation file with curate column in the end
  -v, --version         show program's version number and exit
```

INPUT CNV segmentations file should be a tab-separated file with the following columns:
`chromosome start end gene log2 depth  probes relevance`

The CURATION_ANNOTATION_FILE should be a comma-separated file with the following columns:
`prefix,gene,type,comment`
